...
public interface DetectionCountRepository extends JpaRepository<DetectionCountEntity, Long> {
    ...
    @Query(value = """
            ...
            FROM detection_count
            WHERE
                st_contains(:geometry, location)
            AND detection_time >= :startTimeStamp 
            AND detection_time < :endTimeStamp
        """, nativeQuery = true)
    int countByGeometry(Geometry geometry, ZonedDateTime startTimeStamp, ZonedDateTime endTimeStamp);
